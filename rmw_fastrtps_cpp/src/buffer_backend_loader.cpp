...
void initialize_buffer_backends()
{
  ...
  RCUTILS_LOG_INFO_NAMED(
    "rmw_fastrtps_cpp", "Buffer backends: found %zu backend(s)", backend_names.size());

  for (const auto & backend_name : backend_names) {
    auto backend = registry.get_backend(backend_name);
    if (!backend) {
      RCUTILS_LOG_ERROR_NAMED("rmw_fastrtps_cpp", "Backend '%s' pointer is null", backend_name.c_str());
      continue;
    }

    std::string backend_type = backend->get_backend_type();
    RCUTILS_LOG_INFO_NAMED(
      "rmw_fastrtps_cpp", "Processing backend '%s' (descriptor type: %s)",
    ...
    if (serializers.find(backend_type) != serializers.end()) {
      RCUTILS_LOG_INFO_NAMED(
        "rmw_fastrtps_cpp", "  FastCDR descriptor serializers registered for '%s'",
        backend_type.c_str());
    } else {
  ...
  rosidl_typesupport_fastrtps_cpp::get_endpoint_compatibility_resolver() =
    [](const rmw_topic_endpoint_info_t &, const std::string & backend_type) {
      return rmw_fastrtps_cpp::get_thread_local_backend_compatibility(backend_type);
    };
}
...
void shutdown_buffer_backends()
{
  try {
    ...
    RCUTILS_LOG_ERROR_NAMED(
      "rmw_fastrtps_cpp", "Warning during buffer backend shutdown: %s", e.what());
  }
  ...
  try {
    ...
    RCUTILS_LOG_ERROR_NAMED(
      "rmw_fastrtps_cpp", "Warning clearing backend registry: %s", e.what());
  }
